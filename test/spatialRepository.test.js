...
test('相交查询通过 ST_Intersects 与参数筛选农业要素', async () => {
  ...
  assert.deepEqual(await repository.findIntersections({ regionCode: '370200', category: 'wheat' }), [{ feature_id: 9 }])
...
test('包含查询使用 ST_Within 且不拼接要素编号', async () => {
  ...
  assert.equal(await repository.isFeatureWithinRegion({ featureId: 'parcel-88', regionCode: '370200' }), true)
  assert.match(client.calls[0].text, /ST_Within\s*\(/i)
  assert.deepEqual(client.calls[0].values, ['parcel-88', '370200'])
  assert.equal(client.calls[0].text.includes('parcel-88'), false)
})
...
test('面积统计使用 ST_Intersection 且转换 geography 计算椭球面积', async () => {
  ...
  assert.deepEqual(await repository.calculateIntersectionArea({ regionCode: '370200', category: 'corn' }), {
    featureCount: 2,
